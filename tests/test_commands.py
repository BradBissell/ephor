...
Asserts that every BINDING in CcoApp surfaces as a palette entry (minus
...
from claude_orchestrator.tui.commands import _PALETTE_SKIP_ACTIONS, CcoCommands
...
def test_cco_commands_is_registered_on_app() -> None:
    ...
    assert CcoCommands in CcoApp.COMMANDS
...
async def test_every_binding_surfaces_in_palette(populated_dir: Path) -> None:
    ...
    async with app.run_test() as pilot:  # type: ignore[arg-type]
        await pilot.pause()
        provider = CcoCommands(screen=app.screen)
        ...
        surfaced_actions = {
            b.action for b in CcoApp.BINDINGS if b.action not in _PALETTE_SKIP_ACTIONS
        }
...
async def test_speak_mode_command_label_reflects_current_state(
    ...
    from claude_orchestrator.speech_settings import SPEAK_MODE_FULL, SPEAK_MODE_SUMMARY
