"""Persistent settings for the ephor speech subsystem.

Three independent control surfaces, resolved in priority order:

  1. **Env var** ``EPHOR_TTS_ENABLED`` (``0``/``1``/``true``/``false``/``on``/``off``)
     — wins over everything; lets you launch with a one-shot override
     without touching disk. Useful for ``EPHOR_TTS_ENABLED=0 ephor tui`` when
     you're in a meeting.
  2. **Persisted file** at ``$XDG_CONFIG_HOME/ephor/speech.json``
     — what the TUI's ``m`` hotkey and the ``ephor speech enable|disable``
     CLI write. Survives restarts.
  3. **Default**: enabled iff a kokoro pipeline is detected on disk.
     A user without TTS installed should never have ephor try to play
...
ENV_VAR = "EPHOR_TTS_ENABLED"
MODE_ENV_VAR = "EPHOR_TTS_MODE"
...
class SettingsSource(Enum):
    """Which layer determined the effective state — surfaced by `ephor speech
...
class SpeechSettings:
    ...
    # durations. None until ephor has at least one natural completion. Used
...
def settings_path() -> Path:
    """Where the persisted state lives. Honors ``EPHOR_CONFIG_DIR`` (sandboxes
    ...
    return Path(xdg).expanduser() / "ephor" / "speech.json"
...
def load() -> SpeechSettings:
    ...
    only governs whether ephor SPEAKS, not how it estimates rate."""
...
def save(
    ...
    *,
    ...
    (used by `ephor speech reset-calibration`). ``speak_mode`` accepts
...
def _default_enabled() -> bool:
    ...
    """
    try:
        from ephor.speech_player import kokoro_available
