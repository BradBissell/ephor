"""Command-palette provider for the cco TUI.

Surfaces every keybinding in :class:`CcoApp` as a searchable command so a
...
class CcoCommands(Provider):
    """Expose every CcoApp action as a command-palette entry."""

    @property
    def cco_app(self) -> CcoApp:
        # ``self.app`` is typed as ``App`` upstream — narrow it for callers.
        from claude_orchestrator.tui.app import CcoApp

        app = self.app
        assert isinstance(app, CcoApp)
        return app
...
def _action_runner(app: CcoApp, action: str) -> _CommandRunner:
...
def _label_for(app: CcoApp, binding: Binding) -> tuple[str, str, str | None]:
...
def _state_aware_label(
    app: CcoApp, action: str, fallback_description: str
) -> tuple[str, str | None]:
    ...
    if action == "toggle_speak_mode":
        from claude_orchestrator.speech_settings import SPEAK_MODE_FULL
